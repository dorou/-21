...
void copy_list(list_t * const lst);             // Копировать список
...
//копирование списка
void copy_list(list_t * const lst)
{
node* my_copy(node* head){
    assert(head != NULL);
    node i_p = head;
    node c_l = new node(i_p->item,i_p->next)
    node new_head = c_l;
       while( i_p->next != NULL ){
        node c_l->next = new node(i_p->next->item,i_p->next->next);
        i_ptr = i_p->next;
        c_l = c_l->next;
                                   }
     return new_first;
